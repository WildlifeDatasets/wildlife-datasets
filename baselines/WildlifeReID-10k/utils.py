...
def predict(df, features, col_label="identity", col_split="split"):
    ...
    for dataset_name in df["dataset"].unique():
        ...
        idx_true, idx_pred, similarity = compute_predictions(
            features[idx_test], features[idx_train], return_similarity=True
        )
...
def make_radar_plot2(
    df, color, cols, title=None, figsize=(9, 9), fontsize=16, rotation=4.0, line_width=2, s=0.7, file_name=None
):
    pi = np.pi
    categories = df.index
    N = len(categories)
...
def greedy_similarity_clustering(similarity_matrix: np.ndarray, similarity_threshold: float) -> list:
    ...
    edges = [(i, j, similarity_matrix[i][j]) for i in range(n) for j in range(i + 1, n)]
