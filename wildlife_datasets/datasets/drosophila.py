...
class Drosophila(DownloadURL, WildlifeDataset):
    ...
    def create_catalogue(self) -> pd.DataFrame:
        ...
        for i_week in range(1, 4):
            ...
            data.loc[idx1 & ~idx2, "identity"] = ((i_week - 1) * 20 + folders.loc[idx1 & ~idx2, 1].astype(int)).astype(
                str
            )
            data.loc[idx1 & idx2 & ~idx3, "identity"] = (
                (i_week - 1) * 20 + folders.loc[idx1 & idx2 & ~idx3, 2].astype(int)
            ).astype(str)
