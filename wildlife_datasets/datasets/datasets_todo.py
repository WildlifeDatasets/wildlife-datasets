...
class DatasetFactory():
    ...
    def remove_columns(self, df):
        # TODO: rename to remove_constant_columns
        # TODO: Why would i need this?
        # TODO: df.drop(columns=df.columns[df.astype('str').nunique()==1], inplace=True)
...
class DatasetFactory2(DatasetFactory):
    def __init__(self, root, df, download=False, **kwargs):
        self.root = root

        if download and hasattr(self, 'downloader'): 
            self.downloader(output='data')
        if df is None:
            self.df = self.create_catalogue(**kwargs)
        else:
            self.df = df

class Test(DatasetFactory2):
    # AerialCattle2017
    downloader = download_scripts._example.get_data
...
class Drosophila(DatasetFactory):
    ...
    def create_catalogue(self):
        ...
        # TODO: This looks insanely complicated. how does it work? 
...
class HappyWhale(DatasetFactory):
    ...
    def create_catalogue(self):
        ...
        df2 = { # TODO: rename to df_test and df_train
...
class HumpbackWhaleID(DatasetFactory):
    ...
    def create_catalogue(self):
        ...
        test_files = list(test_files['file']) # TODO: this is not needed
        test_files = pd.Series(np.sort(test_files)) # TODO: sorting is not needed?
...
class SMALST(DatasetFactory):
    ...
    def create_catalogue(self):
        ...
        path0 = masks['file'].str.strip('zebra_') #TODO: why path0?
...
class WNIGiraffes(DatasetFactory):
    ...
    def create_catalogue(self):
        ...
        data = pd.merge(data, df_keypoints, how='left', on='file') # TODO: what if there is one to many match?
    ...
    def extract_keypoints(self, row): # TODO: i would rather keep the keypoints in different (key, value) format
